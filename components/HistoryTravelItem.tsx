...
const HistoryTravelItem: FC<{
  ...
  return (
    ...
    >
      <View className={"h-24 w-24 bg-[#292929] shadow-lg items-center justify-center"}>
        <Ionicons name="image-outline" size={24} color="#A7A7A7" />
      </View>
      <View className={'p-3 space-y-2'}>
        <Text className={"text-white font-semibold"} numberOfLines={1}>{travel?.title}</Text>
        <View>
          <Text className={"text-white text-xs font-semibold"}>预算 {travel.available}/{travel?.budget} ({Math.floor(travel.available / travel.budget * 100)}%)</Text>

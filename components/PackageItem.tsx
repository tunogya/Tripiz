...
const PackageItem = ({ purchasePackage }) => {
  ...
  const other = () => (
    <View className={"bg-[#242424] rounded-lg overflow-hidden relative"}>
      ...
      <View className={"p-4"}>
        <Text className={"text-xs text-[#A7A7A7] text-center"}>每月 58
          人民币。方案将每月自动续期，直到你取消为止。</Text>
      </View>
    </View>
  )
  ...
  return (
    <View className={"px-4"}>
      <View className={"bg-[#242424] rounded-lg overflow-hidden relative"}>
        ...
        <View className={"px-4 py-2"}>
          <Text className={"text-[#F8D4D7] text-2xl font-bold"}>{product.title}</Text>
        </View>
        <View className={"px-4 py-2 space-y-1"}>
          <View className={"flex flex-row items-center space-x-2"}>
            <View className={"w-1.5 h-1.5 bg-white rounded-full mx-1"}></View>
        ...
        <View className={"p-4"}>
          <Text className={"text-xs text-[#A7A7A7] text-center"}>免费试用 1 个月，之后每月 {product.priceString}
            。方案将每月自动续期，直到你取消为止。</Text>
        </View>
      </View>
    </View>
  )

...
const PackageItem = ({ purchasePackage }) => {
  ...
  const isStandard =  product.identifier === "ai.abandon.tripiz.monthly.premium.standard"
    ...
    <View className={"px-4"}>
      <View className={"bg-[#242424] rounded-lg overflow-hidden relative"}>
        ...
        <View className={"px-4 py-2"}>
          <Text className={`${isStandard ? "text-[#F8D4D7]" : "text-[#A9BACF]"} text-2xl font-bold`}>{product.title}</Text>
        </View>
        ...
        <View className={"p-4"}>
          <Text className={"text-xs text-[#A7A7A7] text-center"}>
            {isStandard ? "免费试用 1 个月，之后" : ""}每月 {product.priceString}。方案将每月自动续期，直到你取消为止。</Text>
        </View>
      </View>
    </View>
  )

...
const CommentShowItem: FC<{
  ...
  return (
    <View className={"px-4 pt-4 flex flex-row space-x-3 bg-[#121212]"}>
      {number !== undefined && item.pubkey && publicKey ? (
        <Image
          contentFit={"cover"}
          cachePolicy={"memory-disk"}
          source={{
            uri: `https://www.larvalabs.com/cryptopunks/cryptopunk${(item.pubkey === publicKey ? avatar : number || 0)?.toString().padStart(4, "0")}.png`,
          }}

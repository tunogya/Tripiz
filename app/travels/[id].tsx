...
export default function Page() {
  ...
  const calculateTimeLeft = () => {
    const now = moment();
    const duration = moment.duration(moment(travel.timestamp.end * 1000).diff(now));
    ...
    >
      <View className={"flex space-y-1 pb-3"}>
        ...
        <Text className={"text-white font-bold text-center"}>
          {timeLeft.days ? `${timeLeft.days}天 ` : ""}{timeLeft.hours}:{timeLeft.minutes}:{timeLeft.seconds}
        </Text>
      </View>
      <ScrollView
      ...
      >
        ...
        <View className={"flex flex-row justify-around"}>
          <View className={"flex space-y-1 items-center w-20"}>
            <View className={"flex flex-row items-center space-x-1"}>
              <View className={"bg-[#1ED760] w-2 h-2 rounded-full"}>
              </View>
        ...
        <View className={"pt-6 space-y-3"}>
          <Text className={"text-white text-center text-lg font-semibold"}>所有任务</Text>
          <FlatList
            data={travel.taskIds}
            numColumns={3}
            scrollEnabled={false}
            ...
            renderItem={({item}) => (
              <View className={"flex w-[30%] m-1.5 space-y-0.5"}>
                ...
                <Text className={"text-center text-[#A7A7A7] text-xs"}>{item}</Text>
              </View>
            )}
          />
        </View>
